...
/**
 * Home — real-time market dashboard.
 *
 ...
 *
 * Everything is client-side via react-query so the page shell loads instantly
 * and each block hydrates independently. The "What's Driving" block is the
 * only expensive call; server-side Supabase cache makes refetches cheap.
 */
...
function MarketDriverCard() {
  const q = useQuery({
    ...
    refetchInterval: 5 * 60_000,   // page-level refetch; server-side cache is 15 min
    staleTime: 3 * 60_000,
  });
  const d = q.data;
    ...
    <div className="card card-compact space-y-2">
      ...
      <div className="space-y-1">
        {sorted.map((s) => {
          ...
          const width = Math.abs(pct) / maxAbs * 50;  // % of half-width; center bar
          const isUp = pct >= 0;
          return (
...
function VolLandscapeSnapshot() {
  ...
  const d = q.data;
  // vol-landscape shape varies; render the top 3 entries defensively.
