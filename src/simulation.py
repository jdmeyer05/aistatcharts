...
def predict_30d_random_forest(px_close: pd.Series, n_estimators: int = 200, lookback_days: int = 1000):
    ...
    Generates a forecast path + uncertainty bands based on tree variance.
    """
    if len(px_close) < 100:
    ...
    for _ in range(T):
        ...
        p_mean = np.mean(tree_preds)
        p_std = np.std(tree_preds)
        ...
        new_row = {
            ...
            'lag_3': temp_close[-3],
            'lag_5': temp_close[-5],
            'lag_10': temp_close[-10],
            'MA5': np.mean(temp_close[-5:]),
            'MA20': np.mean(temp_close[-20:]),
            'Vol20': np.std(np.diff(np.log(temp_close[-21:]))) if len(temp_close) > 21 else 0.02
        }
        current_features = pd.DataFrame([new_row])

    # 4. Construct "Fan" based on tree uncertainty
