...
if data is not None and not data.empty:
    ...
    # --- 1. 30-DAY TACTICAL CHART (Deep Random Forest) ---
    st.subheader("🤖 30-Day Tactical Projection (Deep Random Forest)")
    ...
    if len(dates_30d) > 0:
        ...
        # Uncertainty Band (based on tree variance)
        ...
        c2.metric("30-Day RF Target", f"${p50_30[-1]:,.2f}")
        c3.metric("AI Predicted Move", f"{((p50_30[-1]/last_price)-1)*100:.2f}%")
        c4.metric("Lower Bound (P5)", f"${p5_30[-1]:,.2f}")

    st.divider()

    # --- 2. YEAR-END STRATEGIC CHART (Robust Seasonal Monte Carlo) ---
    st.subheader("🎯 Year-End Strategic Projection (Robust Seasonal)")
    
    # Using 3 years (1095 days) for lookback to capture meaningful seasonal cycles
