...
st.set_page_config(page_title="Log In | AI Statcharts", layout="centered",
...
# Already logged in (not a guest) — go to Summary
email = st.session_state.get("user_email", "")
if email and email not in ("", "local-dev@preview", "guest@open-beta"):
...
if not supabase:
    st.info("Authentication is not configured in this environment. All features are available without an account.")
    st.stop()
...
with tab2:
    with st.form("register_form"):
        ...
        new_password = st.text_input("Password", type="password", help="Must be at least 6 characters.", key="reg_password")
        ...
        accept_terms = st.checkbox("I have read and agree to the User Agreement and Terms of Use", value=False)
        ...
        if submit_register:
            if not accept_terms:
                ...
                try:
                    ...
                    st.success("Account created! Switch to the **Log In** tab to access the platform.")
